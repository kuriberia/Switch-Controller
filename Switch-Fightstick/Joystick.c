...
void parseLine(char *line) {
  ...
  sscanf(line, "%s %s", t, c);
	if (strcasecmp(t, "Button") == 0) {
		...
		target = HAT;
	} else if (strcasecmp(t, "HOLD_LX") == 0) {
		target = HOLD_LX;
	} else if (strcasecmp(t, "HOLD_LY") == 0) {
		target = HOLD_LY;
	} else if (strcasecmp(t, "HOLD_RE") == 0) {
		target = HOLD_L_RELEASE;
	} else {
		target = RELEASE;
	}
	if (strcasecmp(c, "Y") == 0) {
		...
		command = SWITCH_X;
	} else if (strcasecmp(c, "BX") == 0) {
		command = SWITCH_B + SWITCH_X;
